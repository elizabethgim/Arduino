...
// turn on the LED if the distance is between dist_min and dist_max
  if(dist_raw < dist_min || dist_raw > dist_max) {
    ...
    if
      (dist_raw <=200) analogWrite(PIN_LED, abs(255 - (int)((abs(dist_raw-100))/3*7)));
    else 
      analogWrite(PIN_LED, (int)((abs(dist_raw-200))/3*7));
  }
...
float USS_measure(int TRIG, int ECHO)
{
  ...
  if(reading < dist_min || reading > dist_max) return; //reading = 0.0; // return 0 when out of range.
  return reading;
